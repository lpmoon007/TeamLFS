...
export function ScenarioEditor({
  ...
  const dirty = title !== title0 || summary !== (summary0 ?? '') || difficulty !== String(difficulty0);

  const save = async () => {
    setBusy(true);
    setFlash(null);
    const d = Number(difficulty);
    const res = await updateScenario({ scenarioId, title, summary, difficulty: isFinite(d) ? d : undefined });
    setBusy(false);
