...
import { isStaff, isAdmin, setFacilitatorSession, clearFacilitatorSession } from '@/lib/facilitator-session';
import { createSessionCore, type CreateSessionResult } from '@/lib/session-core';
...
async function guard() {
  if (!(await isStaff())) throw new Error('unauthorized'); // console staff only (not play-only leaders)
...
export type { SessionLink, CreateSessionResult } from '@/lib/session-core';
...
export async function createSession(params: {
  ...
  const db = await guard();
  return createSessionCore(db, params);
}
